        ...
        try (Connection connection = databaseManager.openConnection();
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                return resultSet.next()
                        ? Optional.of(AuditEventMapper.map(resultSet))
                        : Optional.empty();
            }
    ...
    public AuditEvent[] findByEntity(String entityType, int entityId) throws SQLException {
        ...
        try (Connection connection = databaseManager.openConnection();
            ...
            throw new SQLException(
                    "Failed to read audit history for " + entityType + ' ' + entityId,
                    exception);
        }
    }
    ...
    public void insert(AuditEvent event) throws SQLException {
        ...
        try (Connection connection = databaseManager.openConnection()) {
            ...
            try {
                ...
                throw new SQLException(
                        "Failed to insert audit event " + event.getEventId(), exception);
            }
        }
    }

    private static void requireExistingTarget(
            Connection connection, String entityType, int entityId) throws SQLException {
        String targetQuery = switch (entityType) {
        ...
        try (PreparedStatement statement = connection.prepareStatement(targetQuery)) {
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                if (!resultSet.next()) {
                    throw new SQLException(
                            "Audit target does not exist: " + entityType + ' ' + entityId);
                }
            }
        }
    }
